...
//		Log.e("AlarmReceiver URI",intent.toUri(0));
		...
		if (intent.getExtras().get("intent").equals(Constants.START_DATA_REQUEST))// && query.getSensorName().equalsIgnoreCase("accelerometer"))
		{
			...
			Log.d(Constants.TAG, "Starting service for " + queryNo);
...
//			File fileSent=(File)intent.getSerializableExtra(Constants.FILENAME);
			Log.d(Constants.TAG, "Stopping service for " + queryNo + " system time " + System.currentTimeMillis() + " and query end time " + query.getEndTime());
